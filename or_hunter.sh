...
# --------------------------------------------------
# AUTO INSTALL REQUIRED GO TOOLS
# --------------------------------------------------

install_if_missing() {
    if ! command -v "$1" >/dev/null 2>&1; then
        echo "[*] Installing $1..."
        go install "$2@latest" || true
    fi
}

install_if_missing subfinder github.com/projectdiscovery/subfinder/v2/cmd/subfinder
install_if_missing httpx github.com/projectdiscovery/httpx/cmd/httpx
install_if_missing nuclei github.com/projectdiscovery/nuclei/v3/cmd/nuclei
install_if_missing katana github.com/projectdiscovery/katana/cmd/katana
install_if_missing gau github.com/lc/gau/v2/cmd/gau
install_if_missing waybackurls github.com/tomnomnom/waybackurls
install_if_missing uro github.com/s0md3v/uro
install_if_missing qsreplace github.com/tomnomnom/qsreplace
install_if_missing unfurl github.com/tomnomnom/unfurl
install_if_missing gf github.com/tomnomnom/gf
install_if_missing ffuf github.com/ffuf/ffuf/v2

# --------------------------------------------------
# AUTO HANDLE LOXS (Clone + Flatten)
# --------------------------------------------------

if [ ! -d "loxs" ] && [ ! -f "payloads/or.txt" ]; then
    echo "[*] Cloning LOXS..."
    git clone https://github.com/coffinxp/loxs.git >/dev/null 2>&1 || true
fi

if [ -d "loxs" ]; then
    echo "[*] Moving LOXS files into Hunter root..."
    shopt -s dotglob
    mv loxs/* . 2>/dev/null || true
    rm -rf loxs
    shopt -u dotglob
fi

# --------------------------------------------------
# AUTO HANDLE PARAMSPIDER
# --------------------------------------------------

if [ ! -d "ParamSpider" ]; then
    echo "[*] Cloning ParamSpider..."
    git clone https://github.com/devanshbatham/ParamSpider.git >/dev/null 2>&1 || true
fi

if [ -f "ParamSpider/requirements.txt" ]; then
    echo "[*] Installing ParamSpider dependencies..."
    pip install -r ParamSpider/requirements.txt >/dev/null 2>&1 || true
fi
